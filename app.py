...
# =========================================
# FUNCTION BREAKOUT TYPE CLASSIFICATION
# (Peter Brandt "Last Full Day Low" framework, via Tech Charts / Aksel Kibar)
# =========================================
def classify_breakout_type(data, lookback_base=40, confirm_days=20, vol_ratio_min=1.3):
    """
    Klasifikasi kualitas breakout menjadi:
      TYPE 1 - MOMENTUM        : tidak pernah retest boundary setelah breakout
      TYPE 2 - RETEST NORMAL   : retest boundary tapi LFD Low tetap utuh
      TYPE 3 - RETEST DALAM    : LFD Low tertembus, tapi masih di atas support/negation
      TYPE 4 - FAILED          : support/negation level tertembus, pola batal
      FRESH (Hari-0)           : breakout baru terjadi di candle terakhir
      "-"                      : tidak ada breakout dalam window deteksi

    Resistance & Support didekati dengan rolling box (highest high / lowest low)
    sepanjang `lookback_base` hari sebelum tiap candle. LFD Low = low candle
    terakhir sebelum hari breakout.
    """
    if len(data) < lookback_base + confirm_days + 5:
        return {"type": "N/A", "resistance": np.nan, "support": np.nan,
                "lfd_low": np.nan, "days_since": np.nan}
    ...
    box_high = high.rolling(lookback_base).max().shift(1)
    box_low = low.rolling(lookback_base).min().shift(1)
    vol_avg20 = volume.rolling(20).mean()
    vol_ratio = volume / vol_avg20

    is_breakout = (
        (close > box_high) &
        (close.shift(1) <= box_high.shift(1)) &
        (vol_ratio >= vol_ratio_min)
    )

    breakout_days = is_breakout.tail(confirm_days)
    breakout_dates = breakout_days[breakout_days].index

    if len(breakout_dates) == 0:
        return {"type": "-", "resistance": np.nan, "support": np.nan,
                "lfd_low": np.nan, "days_since": np.nan}

    breakout_date = breakout_dates[-1]
    breakout_idx = data.index.get_loc(breakout_date)

    resistance = box_high.loc[breakout_date]
    support = box_low.loc[breakout_date]
    lfd_low = low.iloc[breakout_idx - 1] if breakout_idx > 0 else np.nan

    days_since = len(data) - 1 - breakout_idx
    current_close = close.iloc[-1]

    if days_since <= 0:
        breakout_type = "FRESH (Hari-0)"
    else:
        post = data.iloc[breakout_idx + 1:]
        min_low_after = post["Low"].min()

        if pd.notna(support) and current_close < support:
            breakout_type = "TYPE 4 - FAILED"
        elif pd.isna(lfd_low) or min_low_after > resistance:
            breakout_type = "TYPE 1 - MOMENTUM"
        elif min_low_after > lfd_low:
            breakout_type = "TYPE 2 - RETEST NORMAL"
        elif pd.notna(support) and min_low_after > support:
            breakout_type = "TYPE 3 - RETEST DALAM"
        else:
            breakout_type = "TYPE 4 - FAILED"

    return {
        "type": breakout_type,
        "resistance": round(float(resistance), 2) if pd.notna(resistance) else np.nan,
        "support": round(float(support), 2) if pd.notna(support) else np.nan,
        "lfd_low": round(float(lfd_low), 2) if pd.notna(lfd_low) else np.nan,
        "days_since": int(days_since) if pd.notna(days_since) else np.nan
    }
...
with st.expander("ℹ️ Tentang Breakout Type 1/2/3/4 (Peter Brandt LFD Low Framework)"):
    st.markdown("""
    - **TYPE 1 - MOMENTUM**: harga tidak pernah balik menguji level resistance setelah breakout. Breakout paling kuat.
    - **TYPE 2 - RETEST NORMAL**: harga balik menguji resistance lama (boleh sedikit tembus intraday), tapi *Last Full Day Low* (low candle terakhir sebelum breakout) tetap utuh.
    - **TYPE 3 - RETEST DALAM**: *Last Full Day Low* tertembus, tapi harga masih bertahan di atas support/negation level (dasar konsolidasi). Sering mengguncang trader stop ketat sebelum lanjut naik.
    - **TYPE 4 - FAILED**: support/negation level tertembus — pola breakout batal.
    - **FRESH (Hari-0)**: breakout baru terjadi di candle terakhir, belum ada data untuk klasifikasi lanjut.
    """)
    ...
    with st.spinner("Menarik data dari TradingView dan melakukan screening..."):
        ...
        for kode in saham_list:
            try:
                ...
                breakout_info = classify_breakout_type(data)
                ...
                # BONUS BREAKOUT TYPE (Peter Brandt LFD Low framework)
                breakout_bonus_map = {
                    "TYPE 1 - MOMENTUM": 8,
                    "TYPE 2 - RETEST NORMAL": 6,
                    "TYPE 3 - RETEST DALAM": 2,
                    "TYPE 4 - FAILED": -8,
                    "FRESH (Hari-0)": 4,
                }
                breakout_bonus = breakout_bonus_map.get(breakout_info["type"], 0)

                final_score = score + market_bonus + commodity_bonus + breakout_bonus
                ...
                hasil.append({
                    ...
                    "Breakout Type": breakout_info["type"],
                    "Resistance": breakout_info["resistance"],
                    "Support/Negation": breakout_info["support"],
                    "LFD Low": breakout_info["lfd_low"],
                    "Hari Sejak BO": breakout_info["days_since"],
        ...
        if not df.empty:
            kolom_urut = [
                "Saham", "Sektor", "Phase", "Score", "Market Bonus", "Sektor Bonus", "Breakout Bonus", "Final Score", "Market Regime", "Quality",
                ...
                "RS", "RS Status", "Volume Ratio", "Volume", "MACD", "STOCH RSI", "Weekly",
                "Breakout Type", "Resistance", "Support/Negation", "LFD Low", "Hari Sejak BO",
                "Liquidity(B)"
            ]
            df = df[kolom_urut]
            ...
            breakout_options = [t for t in df["Breakout Type"].unique().tolist() if t != "-"]
            selected_breakout = st.multiselect(
                "🔎 Filter berdasarkan Breakout Type",
                options=breakout_options,
                default=[]
            )
            df_display = df[df["Breakout Type"].isin(selected_breakout)] if selected_breakout else df
